...
OUTPUT_DIR=""
CONFIG_FILE="${REPO_ROOT}/packages/arcus/ck-subc.json"

PACKAGE_ID="ck-subc"
SOFTWARE_TYPE="service"
ACTION_EXECUTABLE="ck-subc"
...
Usage: $0 [options] [version] [sequence]
  ...
  --output DIR        Output directory (default: dist/<version>/<sequence>/ck-subc)
...
if [ "$SKIP_BUILD" -eq 0 ]; then
  ...
  (cd "$REPO_ROOT" && cargo build --release -p subc-core --bin "$ACTION_EXECUTABLE")
fi
