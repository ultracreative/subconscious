...
OUTPUT_DIR=""
CONFIG_FILE="${REPO_ROOT}/packages/arcus/ck-uc-discussions.json"

PACKAGE_ID="ck-uc-discussions"
SOFTWARE_TYPE="service"
ACTION_EXECUTABLE="ck-uc-discussions"
...
Usage: $0 [options] [version] [sequence]
  ...
  --output DIR        Output directory (default: dist/<version>/<sequence>/ck-uc-discussions)
...
if [ "$SKIP_BUILD" -eq 0 ]; then
  ...
  (cd "$REPO_ROOT" && cargo build --release -p uc-discussions --bin "$ACTION_EXECUTABLE")
fi
